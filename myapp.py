...
class CylindersCard(MDBoxLayout):
    cylinder_name = StringProperty("")
    six_kg_count = StringProperty("")
    thirteen_kg_count = StringProperty("")
    three_kg_count = StringProperty("")
    fifty_kg_count = StringProperty("")
    on_edit = ObjectProperty(allownone = True)
    ...
    def __init__(self, **kwargs):
        ...
        self.name_label = MDLabel(theme_text_color="Custom", text_color="navy", bold = True, theme_font_size = "Custom", font_size = sp(16), halign = "center")
        self.six_count_label = MDLabel(theme_text_color="Custom", bold = True, text_color="cyan", theme_font_size = "Custom", font_size = sp(16), halign = "center")
        self.thirteen_count_label = MDLabel(theme_text_color="Custom", text_color="cyan", bold = True, theme_font_size = "Custom", font_size = sp(16), halign = "center")
        self.three_count_label = MDLabel(theme_text_color="Custom", text_color="cyan", bold = True, theme_font_size = "Custom", font_size = sp(16), halign = "center")
        self.fifty_count_label = MDLabel(theme_text_color="Custom", bold = True, text_color="cyan", theme_font_size = "Custom", font_size = sp(16), halign = "center")


        self.add_widget(self.name_label)
        self.add_widget(self.six_count_label)
        self.add_widget(self.thirteen_count_label)
        self.add_widget(self.three_count_label)
        self.add_widget(self.fifty_count_label)
        self.edit_cylinder_btn = MDIconButton(icon = "pencil-outline", on_release = lambda x: self.on_edit())
        self.add_widget(self.edit_cylinder_btn)

        self.bind(cylinder_name=lambda inst, val: setattr(self.name_label, 'text', val))
        self.bind(six_kg_count=lambda inst, val: setattr(self.six_count_label, 'text', val))
        self.bind(thirteen_kg_count=lambda inst, val: setattr(self.thirteen_count_label, 'text', val))
        self.bind(three_kg_count=lambda inst, val: setattr(self.three_count_label, 'text', val))
        self.bind(fifty_kg_count=lambda inst, val: setattr(self.fifty_count_label, 'text', val))

class TotalCylindersCard(MDBoxLayout):
    cylinder_name = StringProperty("")
    six_kg_count = StringProperty("")
    thirteen_kg_count = StringProperty("")
    three_kg_count = StringProperty("")
    ...
    def __init__(self, **kwargs):
        ...
        self.add_widget(self.three_count_label)
        self.add_widget(self.fifty_count_label)
        self.edit_cylinder_space = MDLabel(size_hint_x = None, width = dp(40))
        self.add_widget(self.edit_cylinder_space)
...
class POS(MDApp):
    ...
    def build(self):
        return Builder.load_file(resource_path('pos.kv'))
    ...
    def add_purchase(self, user_id, prod_id, unit_price, receipt_id, *args):
        ...
        if not hasattr(self, 'receipt_layout'):
            ...
            self.total_value = 0
            self.receipt_layout.add_widget(MDLabel(text='PATO ELECTRICALS', size_hint_y=None, height=dp(30), halign='center'))
    ...
    def show_cylinders(self):
        ...
        ad_header.disabled = False
        ad_header.size_hint_y = None
        ad_header.height = dp(220)
        ad_header.orientation = "vertical"
        ad_header.spacing = dp(10)

        footer.opacity = 1
        footer.disabled = False
        footer.size_hint_y = None
        footer.height = dp(150)
        footer.orientation = "vertical"
        footer.spacing = dp(10)
        
        buttons.height = dp(0)
        buttons.size_hint_y = None
        buttons.opacity = 0
        buttons.disabled = True
        self.root.ids.prev_fab_btn.disabled = True
        footer.clear_widgets()
        ad_header.clear_widgets()

        action_layout = MDBoxLayout(spacing=dp(10), size_hint_y=None, height=dp(60), padding=dp(2))
        self.location_name = MDTextField(MDTextFieldHintText(text='Location'))
        self.cylinder_name = MDTextField(MDTextFieldHintText(text='Cylinder Type'))
        self.six_kg_count = MDTextField(MDTextFieldHintText(text='6Kg '), input_filter = "int")
        self.thirteen_kg_count = MDTextField(MDTextFieldHintText(text='13Kg '), input_filter = "int")
        self.three_kg_count = MDTextField(MDTextFieldHintText(text='3Kg '), input_filter = "int")
        self.fifty_kg_count = MDTextField(MDTextFieldHintText(text='50Kg '), input_filter = "int")
        action_layout.add_widget(self.location_name)
        action_layout.add_widget(self.cylinder_name)
        action_layout.add_widget(self.six_kg_count)
        action_layout.add_widget(self.thirteen_kg_count)
        action_layout.add_widget(self.three_kg_count)
        action_layout.add_widget(self.fifty_kg_count)
        add_btn = MDButton(MDButtonText(text='Add', bold = True, theme_text_color = "Custom", text_color = "white"), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "green", on_release=lambda x: self.add_cylinders())
        action_layout.add_widget(add_btn)
        ad_header.add_widget(action_layout)
        ad_header.add_widget(MDDivider(color = "gold"))
        self.head_text = MDLabel(text='Store', bold=True, size_hint_y=None, height=dp(40), theme_text_color='Custom', text_color='green', theme_font_size='Custom', font_size=sp(25), halign='center')
        ad_header.add_widget(self.head_text)
        ad_header.add_widget(MDDivider(color = "gold"))
        p_header = MDBoxLayout(spacing=dp(10), size_hint_y=None, height=dp(50))
        name = MDLabel(text='Cylinder Name', halign='center', bold=True, theme_text_color = "Custom", text_color = "tomato")
        six_kg = MDLabel(text='6Kg', halign='center', bold=True, theme_text_color = "Custom", text_color = "tomato")
        thirteen_kg = MDLabel(text='13Kg', halign='center', bold=True, theme_text_color = "Custom", text_color = "tomato")
        three_kg = MDLabel(text='3Kg', halign='center', bold=True, theme_text_color = "Custom", text_color = "tomato")
        fifty_kg = MDLabel(text='50Kg', halign='center', bold=True, theme_text_color = "Custom", text_color = "tomato")
        p_header.add_widget(name)
        p_header.add_widget(six_kg)
        p_header.add_widget(thirteen_kg)
        p_header.add_widget(three_kg)
        p_header.add_widget(fifty_kg)
        p_header.add_widget(MDLabel(text='', halign='center', size_hint_x=None, width=dp(40)))
        ad_header.add_widget(p_header)
        p_footer = MDBoxLayout(spacing = dp(5), size_hint_y = None, height = dp(40))
        total_label = MDLabel(text = "Total", bold = True, halign = "center", theme_text_color = "Custom", text_color = "magenta")
        self.six_kg_label = MDLabel(text = "0", bold = True, halign = "center", theme_text_color = "Custom", text_color = "blue")
        self.thirteen_kg_label = MDLabel(text = "0", bold = True, halign = "center", theme_text_color = "Custom", text_color = "blue")
        self.three_kg_label = MDLabel(text = "0", bold = True, halign = "center", theme_text_color = "Custom", text_color = "blue")
        self.fifty_kg_label = MDLabel(text = "0", bold = True, halign = "center", theme_text_color = "Custom", text_color = "blue")
        p_footer.add_widget(total_label)
        p_footer.add_widget(self.six_kg_label)
        p_footer.add_widget(self.thirteen_kg_label)
        p_footer.add_widget(self.three_kg_label)
        p_footer.add_widget(self.fifty_kg_label)
        p_footer.add_widget(MDLabel(text='', halign='center', size_hint_x=None, width=dp(40)))
        footer.add_widget(p_footer)

        filter_section = MDBoxLayout(size_hint_y=None, height=dp(50), spacing=dp(40))
        store_btn = MDButton(MDButtonText(text='Store', theme_text_color = "Custom", text_color = "white", bold = True), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "orange", on_release=lambda x: self.view_cylinders('Store'))
        shop1_btn = MDButton(MDButtonText(text='Shop1', theme_text_color = "Custom", text_color = "white", bold = True), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "brown", on_release=lambda x: self.view_cylinders('Shop1'))
        shop2_btn = MDButton(MDButtonText(text='Shop2', theme_text_color = "Custom", text_color = "white", bold = True), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "purple", on_release=lambda x: self.view_cylinders('Shop2'))
        shop3_btn = MDButton(MDButtonText(text='Shop3', theme_text_color = "Custom", text_color = "white", bold = True), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "teal", on_release=lambda x: self.view_cylinders('Shop3'))
        total_btn = MDButton(MDButtonText(text='Aggregate', theme_text_color = "Custom", text_color = "white", bold = True), pos_hint={'center_y': 0.5}, theme_bg_color = "Custom", md_bg_color = "magenta", on_release=lambda x: self.view_cylinders_total())
        filter_section.add_widget(Widget())
        filter_section.add_widget(store_btn)
        filter_section.add_widget(shop1_btn)
        filter_section.add_widget(shop2_btn)
        filter_section.add_widget(shop3_btn)
        filter_section.add_widget(total_btn)
        filter_section.add_widget(Widget())
        footer.add_widget(filter_section)
        self.view_cylinders('Store')

    def add_cylinders(self):
        name = self.cylinder_name.text
        loca = self.location_name.text
        six = self.six_kg_count.text
        thirteen = self.thirteen_kg_count.text
        three = self.three_kg_count.text
        fifty = self.fifty_kg_count.text
        new_cylinder = database.Cylinder(
            cylinder_name=name.strip().lower(), 
            cylinder_loc=loca.strip().lower(), 
            six_kg_count=int(six.strip()) if six.strip() else 0, 
            thirteen_kg_count=int(thirteen.strip()) if thirteen.strip() else 0, 
            three_kg_count=int(three.strip()) if three.strip() else 0, 
            fifty_kg_count=int(fifty.strip()) if fifty.strip() else 0)
        database.session.add(new_cylinder)
        database.session.commit()

    def view_cylinders(self, locat):
        cylinders = database.session.query(database.Cylinder).filter_by(cylinder_loc=locat.lower()).all()

        prev = self.root.ids.admin_prev
        prev.md_bg_color = 1, 1, 1, 1
        self.root.ids.rvb.default_size = (None, dp(50))
        data = []
        prev.viewclass = "CylindersCard"
        for cylinder in cylinders:
            data.append({
                'viewclass': "CylindersCard",
                'cylinder_name': cylinder.cylinder_name.capitalize(),
                'six_kg_count': str(cylinder.six_kg_count),
                'thirteen_kg_count': str(cylinder.thirteen_kg_count),
                'three_kg_count': str(cylinder.three_kg_count),
                'fifty_kg_count': str(cylinder.fifty_kg_count),
                'on_edit': lambda store_name = cylinder.cylinder_loc, cylinder_name = cylinder.cylinder_name: self.edit_cylinders(store_name, cylinder_name),
            })
        self.head_text.text = locat.capitalize()
        totals = database.session.query(
            func.sum(database.Cylinder.six_kg_count),
            func.sum(database.Cylinder.thirteen_kg_count),
            func.sum(database.Cylinder.three_kg_count),
            func.sum(database.Cylinder.fifty_kg_count),
        ).filter_by(cylinder_loc=locat.lower()).first()

        self.six_kg_label.text = str(totals[0] or 0)
        self.thirteen_kg_label.text = str(totals[1] or 0)
        self.three_kg_label.text = str(totals[2] or 0)
        self.fifty_kg_label.text = str(totals[3] or 0)        
        prev.data = data

    def view_cylinders_total(self):
        prev = self.root.ids.admin_prev
        prev.md_bg_color = 1, 1, 1, 1
        self.root.ids.rvb.default_size = (None, dp(50))

        cylinder_names = (
            database.session.query(database.Cylinder.cylinder_name)
            .distinct()
            .all()
        )
        cylinder_names = [name[0] for name in cylinder_names]

        totals_query = (
            database.session.query(
                database.Cylinder.cylinder_name,
                database.Cylinder.cylinder_loc,
                func.sum(database.Cylinder.six_kg_count),
                func.sum(database.Cylinder.thirteen_kg_count),
                func.sum(database.Cylinder.three_kg_count),
                func.sum(database.Cylinder.fifty_kg_count),
            )
            .group_by(database.Cylinder.cylinder_name, database.Cylinder.cylinder_loc)
            .all()
        )

        totals_dict = {}
        for name, loc, six, thirteen, three, fifty in totals_query:
            if name not in totals_dict:
                totals_dict[name] = {}
            totals_dict[name][loc] = {
                "6Kg": six or 0,
                "13Kg": thirteen or 0,
                "3Kg": three or 0,
                "50Kg": fifty or 0,
        ...
        for name in cylinder_names:
            store = totals_dict.get(name, {}).get("store", {"6Kg": 0, "13Kg": 0, "3Kg": 0, "50Kg": 0})
            shop1 = totals_dict.get(name, {}).get("shop1", {"6Kg": 0, "13Kg": 0, "3Kg": 0, "50Kg": 0})
            shop2 = totals_dict.get(name, {}).get("shop2", {"6Kg": 0, "13Kg": 0, "3Kg": 0, "50Kg": 0})
            shop3 = totals_dict.get(name, {}).get("shop3", {"6Kg": 0, "13Kg": 0, "3Kg": 0, "50Kg": 0})

            data.append({
                'viewclass': "TotalCylindersCard",
                'cylinder_name': name.capitalize(),
                'six_kg_count': str(store["6Kg"] + shop1["6Kg"] + shop2["6Kg"] + shop3["6Kg"]),
                'thirteen_kg_count': str(store["13Kg"] + shop1["13Kg"] + shop2["13Kg"] + shop3["13Kg"]),
                'three_kg_count': str(store["3Kg"] + shop1["3Kg"] + shop2["3Kg"] + shop3["3Kg"]),
                'fifty_kg_count': str(store["50Kg"] + shop1["50Kg"] + shop2["50Kg"] + shop3["50Kg"]),
            })

        self.head_text.text = "Total Cylinders"
        prev.viewclass = "TotalCylindersCard"
        prev.data = data
    ...
    def gas_dialog(self, *args):
        self.gas_location = MDTextField(MDTextFieldHintText(text='Locaton'))
        self.gas_type = MDTextField(MDTextFieldHintText(text='Cylinder Name'))
        self.six_kg_num = MDTextField(MDTextFieldHintText(text='6Kg'), input_filter = "int")
        self.thirteen_kg_num = MDTextField(MDTextFieldHintText(text='13Kg'), input_filter = "int")
        self.three_kg_num = MDTextField(MDTextFieldHintText(text='3Kg'), input_filter = "int")
        self.fifty_kg_num = MDTextField(MDTextFieldHintText(text='50Kg'), input_filter = "int")
        container = MDDialogContentContainer(orientation='vertical', spacing=dp(15))
        container.add_widget(self.gas_location)
        container.add_widget(self.gas_type)
        container.add_widget(self.six_kg_num)
        container.add_widget(self.thirteen_kg_num)
        container.add_widget(self.three_kg_num)
        container.add_widget(self.fifty_kg_num)
        self.cylinder_dialog = MDDialog(
            MDDialogIcon(icon='gas-cylinder'),
            MDDialogHeadlineText(text='Sell Gas Cylinders?'), 
            container, 
            MDDialogButtonContainer(
                Widget(), 
                MDButton(MDButtonText(text='Sell'), on_release=self.modify_cylinders), 
                MDButton(MDButtonText(text='Cancel'), on_release=lambda x: self.cylinder_dialog.dismiss()), 
                spacing=dp(10)
            )
        )
        self.cylinder_dialog.open()

    def modify_cylinders(self, *args):
        loca = self.gas_location.text.strip() if self.gas_location.text.strip() else "store"
        name = self.gas_type.text.strip() if self.gas_type.text.strip() else "name"

        six = int(self.six_kg_num.text.strip()) if self.six_kg_num.text.strip() else 0
        thirteen = int(self.thirteen_kg_num.text.strip()) if self.thirteen_kg_num.text.strip() else 0
        three = int(self.three_kg_num.text.strip()) if self.three_kg_num.text.strip() else 0
        fifty = int(self.fifty_kg_num.text.strip()) if self.fifty_kg_num.text.strip() else 0

        gas = database.session.query(database.Cylinder).filter(
            (database.Cylinder.cylinder_name == name.lower()) &
            (database.Cylinder.cylinder_loc == loca.lower())
        ).first()

        if not gas:
            self.show_snack(f"{name} doesn't exist!")
            return

        if gas.six_kg_count >= six:
            gas.six_kg_count -= six
        else:
            self.show_snack(f"Insufficient 6Kg {gas.cylinder_name} Cylinder")
            return

        if gas.thirteen_kg_count >= thirteen:
            gas.thirteen_kg_count -= thirteen
        else:
            self.show_snack(f"Insufficient 13Kg {gas.cylinder_name} Cylinder")
            return

        if gas.three_kg_count >= three:
            gas.three_kg_count -= three
        else:
            self.show_snack(f"Insufficient 3Kg {gas.cylinder_name} Cylinder")
            return

        if gas.fifty_kg_count >= fifty:
            gas.fifty_kg_count -= fifty
        else:
            self.show_snack(f"Insufficient 50Kg {gas.cylinder_name} Cylinder")
            return

        database.session.commit()
        self.show_snack("Cylinders updated successfully")


    def edit_cylinders(self, store_name, gas_name):
        gas = database.session.query(database.Cylinder).filter((database.Cylinder.cylinder_loc == store_name) & (database.Cylinder.cylinder_name == gas_name)).first()
        self.gas_name = MDTextField(MDTextFieldHintText(text='Cylinder Name'), text=gas.cylinder_name)
        self.gas_six_kg = MDTextField(MDTextFieldHintText(text='6Kg'), text=str(gas.six_kg_count), input_filter = "int")
        self.gas_thirteen_kg = MDTextField(MDTextFieldHintText(text='13Kg'), text=str(gas.thirteen_kg_count), input_filter = "int")
        self.gas_three_kg = MDTextField(MDTextFieldHintText(text='3Kg'), text=str(gas.three_kg_count), input_filter = "int")
        self.gas_fifty_kg = MDTextField(MDTextFieldHintText(text='50Kg'), text=str(gas.fifty_kg_count), input_filter = "int")
        container = MDDialogContentContainer(orientation='vertical', spacing=dp(10))
        container.add_widget(self.gas_name)
        container.add_widget(self.gas_six_kg)
        container.add_widget(self.gas_thirteen_kg)
        container.add_widget(self.gas_three_kg)
        container.add_widget(self.gas_fifty_kg)
        self.edit_cylinder_dialog = MDDialog(
            MDDialogIcon(icon='pencil-outline'), 
            MDDialogHeadlineText(text='Edit Gas Cylinder?'), 
            MDDialogSupportingText(text='Modify Cylinder details below'), 
            container, 
            MDDialogButtonContainer(
                Widget(), 
                MDButton(MDButtonText(text='Edit'), on_release=lambda x: self.edit_gas(store_name, gas_name)), 
                MDButton(MDButtonText(text='Cancel'), on_release=lambda x: self.edit_cylinder_dialog.dismiss()), 
                spacing=dp(20)
            )
        )
        self.edit_cylinder_dialog.open()

    def edit_gas(self, store_name, cylinder_name):
        gas = database.session.query(database.Cylinder).filter((database.Cylinder.cylinder_loc == store_name) & (database.Cylinder.cylinder_name == cylinder_name)).first()
        if gas:
            gas.cylinder_name = self.gas_name.text.strip()
            gas.six_kg_count = int(self.gas_six_kg.text.strip())
            gas.thirteen_kg_count = int(self.gas_thirteen_kg.text.strip())
            gas.three_kg_count = int(self.gas_three_kg.text.strip())
            gas.fifty_kg_count = int(self.gas_fifty_kg.text.strip())
            database.session.commit()
            self.show_snack(f'{cylinder_name} updated successfully!!')
        else:
            self.show_snack(f"{cylinder_name} doesn't exist!!")
